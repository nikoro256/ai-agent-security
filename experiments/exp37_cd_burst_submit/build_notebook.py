"""exp37 submit ノートブック生成（冪等）: attack_template.py を %%writefile セルに埋め込む。"""

import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
OUT = HERE / "ai-agent-sec-exp37-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp37 Submit: cd_burst8 race（CD burst 効率化テスト）\n"
            "\n"
            "exp34（cd_2msg 純粋 CD、LB 9.405）の burst 化。msg1 を 8 通一斉送信 + harmony prefill にして\n"
            "1候補あたり 34 raw（8 sends x CD4 + cell2）を狙う。gemma では 2 通に崩壊するので race が\n"
            "cd_burst2（10 raw）を自然選択。fallback は cd_1（exp34 と同じ単発、6 raw）。\n"
            "probe race: burst8 が全 probe 発火かつ raw/trial >= 18 なら無条件選択、それ以外は秒/raw race。"
        ),
