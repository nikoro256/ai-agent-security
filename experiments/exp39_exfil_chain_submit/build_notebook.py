...
OUT = HERE / "ai-agent-sec-exp39-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp39 Submit: EXFIL 連打 race（burst12 / exfil_chain7）\n"
            "\n"
            "exp31（現ベスト 95.220）に gemma 用の exfil_chain7（7msg 連鎖・msg1 に偽 system 注入）を追加。\n"
            "gpt_oss は burst12（130 raw/候補）、gemma は exfil_chain7（114 raw/候補、1msg=1call で\n"
            "gemma の 2回目 tool call 崩壊を回避）。fallback は bare_ok（18 raw/候補）。"
        ),
