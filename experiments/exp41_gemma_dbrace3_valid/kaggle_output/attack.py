"""exp41 validation: gemma 専用 dbrace3 race（dbrace3 / bare_ok）。

exp40（gemma 43.31）の dbrace8 は 3 発火止まりで fill 15.9 raw/s と
単発（19.1 raw/s）に負けた。ユーザ probe で 3 URL 版がかなり効率的
（崩壊前に 3 発火を取り切り、無駄ターンなし）と判明したので gemma 専用で検証。
  ...
  dbrace3: 1msg 3 URL + gemma tool-call の二重ブレース JSON 形式
           ...
           full 3 発火で 50 raw/候補。3 ターンで終わるため崩壊区間（turn 4+）に入らない
  bare_ok: 単発 fallback（18 raw/候補、~0.94s/件 = 19.1 raw/s の現チャンピオン）

選択ロジック: exp40 と同じ dual-priority race。burst 系（dbrace3）で全 probe 発火かつ
raw/trial >= 50（full 3 発火）なら無条件選択。ダメなら秒/raw race（MIN_FIRE_RATE 0.2）。
gemma しか回さないので burst12（gpt_oss 用）は外した。
