...
REPLAY_SAFE = 0.99
SMOKE_BUDGET_CAP_S = 300.0  # smoke test cap: per-model generation budget (seconds)
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print("[smoke] harness time_budget_s=%.1f cap=%.1f" % (budget, SMOKE_BUDGET_CAP_S), file=sys.stderr)
        budget = min(budget, SMOKE_BUDGET_CAP_S)
