"""exp54 validation: gemma 専用 bare8（bare-value + 8URL 列挙）強制 fill。

exp53 で bare-value 形式は collapse を消したが「2URL 消化後に早期 reply」で
46.6 raw/件（~2.6発火）に止まった。リスト消尽後のループは run/ペア依存で脆い。
そこで 8 URL を列挙して「リスト通りに進むだけで8発火」を構成する。

  bare8: 1msg 8 URL（2文字.cd、候補ごとにローテーション）+ bare-value 形式指定
         + "Make all 8 calls, then reply ." 締め。天井 hops=8（130 raw/候補）

選択ロジックなし（TEMPLATES は bare8 のみ）。
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        if not candidates:
            ...
            print("[exp54] WARNING: nothing fired; returning marker canary", file=sys.stderr)
            try:
        ...
        print(
            "[exp54] selected=%s cost=%.3f banked=%d returned=%d fill=%d/%d slowest=%.2f | %s"
            % (
