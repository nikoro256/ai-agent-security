"""exp17: replay-aware race. Selection probes at hops=8 (replay-equivalent latency), fill at hops=1 (fast banking). Fixes exp16: search-cost selection picked replay-pathological templates.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        def trial(template_index: int, index: int, hops: int | None = None) -> tuple[bool, float]:
            ...
            try:
                env.reset()
                env.interact(message, max_tool_hops=hops if hops is not None else max_tool_hops)
        ...
        for _ in range(PROBE_REPS):
            for template_index in range(len(TEMPLATES)):
                if not search_time_left():
                    break
                trial(template_index, probe_index, hops=8)  # exp17: select by replay-equivalent cost
                probe_index += 1
