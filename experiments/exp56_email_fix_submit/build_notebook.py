"""exp56 submit ノートブック生成（冪等）: attack_template.py を %%writefile セルに埋め込む。"""

import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
OUT = HERE / "ai-agent-sec-exp56-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp56 Submit: cd_burst8 + gemma email bare くじ+固定 (CD 安全枠)\n"
            "\n"
            "exp37 の gemma 経路を exp55 email bare バーストに置き換え。\n"
            "gemma: ランダム2文字アドレスを最大40回くじ → 当たり (raw>=22) を固定して fill 量産。\n"
            "gpt_oss: cd_burst8 (exp37 実績 34 raw/件)。\n"
            "exp55 分布: mean 4.76発火・6発火50%・mean 21.0 raw @4.88s。"
        ),
