"""exp34 submit ノートブック生成（冪等）: attack_template.py を %%writefile セルに埋め込む。"""

import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
OUT = HERE / "ai-agent-sec-exp34-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp34 Submit: cd_2msg pure-CONFUSED_DEPUTY test\n"
            "\n"
            "全候補が 2-message 時点ズレ CD（msg1: 自己完結の送信指示 / msg2: benign 締め）。\n"
            "guard は msg1 を見て allow、scorer は最終 msg2 を見て CD 発火（exp33 battery で両モデル 30/30 確認済み）。\n"
            "LB が ~45-60 なら hosted でも CD が点く証明、~0.5 なら canary（marker fallback）、0 なら pipeline 死。"
        ),
