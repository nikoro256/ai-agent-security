...
OUT = HERE / "ai-agent-sec-exp45-varurl-dist.ipynb"
...
TLDS = ("cd", "cg", "co", "cx", "dev", "net")


def make_prompt():
    hosts = []
    for _ in range(2):
        wlen = rng.choice((2, 2, 3))
        host = "".join(rng.choice(string.ascii_lowercase) for _ in range(wlen))
        hosts.append(f"{host}.{rng.choice(TLDS)}")
...
print()
print(f"=== dbrace3r(2url, VARIED hosts) x{N} distribution (gemma, hops=8) ===")
...
def main() -> None:
    ...
    nb["cells"][0] = md_cell(
        "# exp45: dbrace3r(2URL) 発火分布・URL を毎回ランダム化（gemma × 24回）\n"
        "\n"
