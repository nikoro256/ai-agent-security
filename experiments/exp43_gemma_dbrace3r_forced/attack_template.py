"""exp43 validation: gemma 専用 dbrace3r 強制選択（race なし・単一テンプレ）。

exp42 で dbrace3r（dbrace3 + 末尾 "then reply ."）は probe 2/2 が 6-7 発火
（106 raw/trial）と高 raw を示したが、race が dbrace3（効率 21.1 raw/s）を
選んだため fill 相の分布が未計測だった。本番は replay 律速で raw/候補が
効くため、dbrace3r を強制選択して fill の発火分布を直接測る。

  dbrace3r: 1msg 3 URL + double-brace 形式指定 + "then reply ." 締め。
            リスト消尽後も発火が継続する（同一URLループも各回 16 raw 計上）挙動を利用。
            天井は hops=8（最大 130 raw/候補）。懸念は turn 4+ の崩壊分散

ユーザ指定により選択ロジックなし（TEMPLATES は dbrace3r のみ、warmup も同じ）。
gate も実質無意味（通らなくても race で dbrace3r が選ばれる）。
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        if not candidates:
            ...
            print("[exp43] WARNING: nothing fired; returning marker canary", file=sys.stderr)
            try:
        ...
        print(
            "[exp43] selected=%s cost=%.3f banked=%d returned=%d fill=%d/%d slowest=%.2f | %s"
            % (
