...
| exp17 | replay-aware race: **選択 probe を hops=8**（replay 相当コストで選ぶ）・fill は hops=1 のまま高速化。安全6テンプレ | **過去最高**: gpt_oss 55.71（971 banked・**619 replayed** = 従来最高 exp14 の 600 更新、inj_close_noanalysis 選択）、gemma 51.66（749 banked・574 replayed、bare_ok 選択で exp14 576 と互角）。探索コストと replay コストの不連動問題を設計で解決 → 提出（ref 追記） |
