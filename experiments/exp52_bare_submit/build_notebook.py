...
OUT = HERE / "ai-agent-sec-exp52-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp52 Submit: burst12 + gemma ペアくじ量産\n"
            "\n"
            "exp31（95.220）の gpt_oss 経路はそのまま。gemma は dbrace2r ペアくじ\n"
            "（この run で 8 発火する 2文字.cd ペアを探索→量産、縮退は観測最大ペア）。\n"
            "exp48 local 51.73・fill 100%・101.5 raw/候補 検証済み。fallback は marker canary。"
        ),
