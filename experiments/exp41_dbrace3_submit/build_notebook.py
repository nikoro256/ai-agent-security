"""exp39 submit ノートブック生成（冪等）: attack_template.py を %%writefile セルに埋め込む。"""

import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
OUT = HERE / "ai-agent-sec-exp41-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp41 Submit: EXFIL 連打 race（burst12 / dbrace3）\n"
            "\n"
            "exp31（現ベスト 95.220）の gemma 単発 fallback を dbrace3（単一msg 3URL double-brace burst、\n"
            "exp41-valid: 54.4 raw/候補・fill 179/179 発火・20.7 raw/s）に置き換え。\n"
            "gpt_oss は burst12（130 raw/候補）、gemma は dbrace3（~50-66 raw/候補）。fallback は bare_ok。"
        ),
