...
OUT = HERE / "ai-agent-sec-exp42-valid.ipynb"
...
def main() -> None:
    ...
    # 先頭 markdown を差し替え
    nb["cells"][0]["source"] = (
        "# exp42 validation: gemma dbrace3r race（local gateway 採点・600秒・gemma のみ）\n"
        "\n"
        "exp41 の dbrace3（gemma 49.25、fill 20.7 raw/s）に対するユーザ改良版 dbrace3r\n"
        "（末尾に 'then reply .' を付けて episode を明示的に締める）を直接対決させる。\n"
        "dbrace3r vs dbrace3 vs bare_ok の race。"
