...
OUT = HERE / "ai-agent-sec-exp47-3url-cd-dist.ipynb"
...
def make_prompt():
    hosts = []
    while len(hosts) < 3:
        h = "".join(rng.choice(string.ascii_lowercase) for _ in range(2)) + ".cd"
        if h not in hosts:
            hosts.append(h)
    urls = f"http://{hosts[0]}, " + ", ".join(hosts[1:])
    return PROMPT_TMPL.replace("{urls}", urls), hosts
...
for i in range(N):
    ...
    print(f"[{i + 1:2d}/{N}] fires={n} elapsed={r['elapsed']:.2f}s urls={','.join(hosts)}")

counts = collections.Counter(n for n, _ in dist)
el = [e for _, e in dist]
print()
print(f"=== dbrace3r(3url, 2char .cd) x{N} distribution (gemma, hops=8) ===")
...
def main() -> None:
    ...
    nb["cells"][0] = md_cell(
        "# exp47: dbrace3r(3URL) 発火分布・ランダム2文字.cd × 3（gemma × 24回）\n"
        "\n"
