"""exp38 submit ノートブック生成（冪等）: attack_template.py を %%writefile セルに埋め込む。"""

import json
from pathlib import Path

HERE = Path(__file__).resolve().parent
OUT = HERE / "ai-agent-sec-exp38-submit.ipynb"
...
def main() -> None:
    ...
    cells = [
        md_cell(
            "# Exp38 Submit: CD 連打 race（burst8 / msgchain8）\n"
            "\n"
            "exp37（cd_burst8、LB 18.310）に gemma 用の msgchain8（10msg 連鎖・各 msg に send 意図明示）を追加。\n"
            "gpt_oss は cd_burst8（34 raw/候補）、gemma は cd_msgchain8（34 raw/候補、1msg=1call で\n"
            "gemma の 2回目 tool call 崩壊を回避しつつ per-message 意図ガードも通過）。fallback は cd_1。"
        ),
